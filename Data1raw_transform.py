...
w = 492
h = 492
path = r'./data_set/Data1_492_492_303_uchar.raw'
size = (c,w,h)

rawData = np.fromfile(path, dtype='uint8')
...
# 创建文件夹
# os.makedirs('data_set/Data1')
# os.makedirs('data_set/Data1_noise')
# os.makedirs('./data_set/Data1_concat')

def make_feature():
    for i in range(len(reshapeRawData)):
        im1 = Image.fromarray(reshapeRawData[i])
        im1.save(f'./data_set/Data1/{i}.png')
...
def noise_data(images):
    for i in range(c):
        ...
        output.save(f'./data_set/Data1_noise/{i}.png')

def figure_concat():
    for i in range(c):
        im1 = cv2.imread(f'./data_set/Data1_noise/{i}.png')
        im2 = cv2.imread(f'./data_set/Data1/{i}.png')
        fig = plt.figure(1)
        plt.subplot(1,2,1)
        plt.imshow(im1)
        plt.xticks([])
        ...
        plt.yticks([])
        plt.draw()
        plt.savefig(f'./data_set/Data1_concat/{i}.png')
        plt.pause(0.1)  # 间隔的秒数：0.1s
        plt.close(fig)

if __name__ == '__main__':
    make_feature()
    images_gif = gif_main(r'./data_set/Data1','data1.gif')
    noise_data(images_gif)
    noise_gif = gif_main(r'./data_set/Data1_noise','data1_noise.gif')
    figure_concat()
    concat_gif = gif_main(r'./data_set/Data1_concat','data1_concat.gif')
