...
w = 512
h = 512
path = r'./data_set/Data4_512_512_273_16.raw'
size = (c,w,h)
...
# 创建文件夹
os.makedirs('data_set/Data4')
os.makedirs('data_set/Data4_noise')
os.makedirs('./data_set/Data4_concat')

def make_feature():
    for i in range(len(reshapeRawData)):
        im1 = Image.fromarray(reshapeRawData[i])
        im1.save(f'./data_set/Data4/{i}.png')
...
def noise_data(images):
    for i in range(c):
        ...
        output.save(f'./data_set/Data4_noise/{i}.png')

def figure_concat():
    for i in range(c):
        im1 = cv2.imread(f'./data_set/Data4_noise/{i}.png')
        im2 = cv2.imread(f'./data_set/Data4/{i}.png')
        fig = plt.figure(1)
        plt.subplot(1,2,1)
        plt.imshow(im1)
        plt.xticks([])
        ...
        plt.yticks([])
        plt.draw()
        plt.savefig(f'./data_set/Data4_concat/{i}.png')
        plt.pause(0.1)  # 间隔的秒数：0.1s
        plt.close(fig)

if __name__ == '__main__':
    make_feature()
    images_gif = gif_main(r'./data_set/Data4','data4.gif')
    noise_data(images_gif)
    noise_gif = gif_main(r'./data_set/Data4_noise','data4_noise.gif')
    figure_concat()
    concat_gif = gif_main(r'./data_set/Data4_concat','data4_concat.gif')
