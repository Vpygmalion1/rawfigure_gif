...
# step1：读取源文件，查看维度
path = r'./data_set/Data2_512_512_244_16.raw'
size = (244,512,512)
...
# 创建文件夹
# os.makedirs('data_set/Data2')
# os.makedirs('data_set/Data2_noise')
# os.makedirs('./data_set/Data2_concat')
...
def noise_data(images):
    for image in images:
        output = sp_noise(image,0.1)
        plt.imshow(output)
        output = Image.fromarray(output)
        output.save('./data_set/Data2_noise/{}.png'.format(i for i in range(len(images))))

def figure_concat():
    for i in range(244):
        im1 = cv2.imread(f'./data_set/Data2_noise/{i}.png')
        im2 = cv2.imread(f'./data_set/Data2/{i}.png')
        fig = plt.figure(1)
        plt.subplot(1,2,1)
        plt.imshow(im1)
        plt.xticks([])
        ...
        plt.yticks([])
        plt.draw()
        plt.savefig(f'./data_set/Data2_concat/{i}.png')
        plt.pause(0.1)  # 间隔的秒数：6s
        plt.close(fig)

if __name__ == '__main__':
    images_gif = gif_main(r'./data_set/Data2')
    noise_data(images_gif)
    noise_gif = gif_main(r'./data_set/Data2_noise','noise.gif')
    figure_concat()
    concat_gif = gif_main(r'./data_set/Data2_concat','concat.gif')
